...
class NeandocAssistant {
  constructor(options = {}) {
    ...
    this.watchInterval = null; // Store interval reference for proper cleanup
  }

  async analyzeAndPrompt(codeStructure) {
...
`;
  ...
  async startWatchMode(directory) {
    if (this.isRunning) {
    ...
    // Initial check
    await this.performCheck(directory);
    ...
    this.watchInterval = setInterval(async () => {
      ...
      try {
        ...
        console.error(chalk.red(`❌ Watch check failed: ${error.message}`));
        // Continue running on errors, don't crash the watcher
      }
    }, this.checkInterval);
...
Der Code selbst darf nicht verändert werden.`;
  ...
  async callMCPProtocol() {
    // Implement MCP protocol communication
    throw new Error('MCP protocol not yet implemented');
  }

  // Mock responses for development
  async mockClaudeResponse() {
    // TODO: Implement mock response generation
    return { comments: [] };
  }

  async mockOpenAIResponse() {
    // TODO: Implement mock OpenAI response generation
    return { comments: [] };
  }
}
